...
using MyCourseraWebApp.ViewModels;
...
namespace MyCourseraWebApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register()
        {
            ...
            {
                viewModel.Types = context.ApplicationUserTypes
                    .OrderBy(t => t.Id)
                    .Skip(1)
                    .OrderBy(t => t.Name)
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { FullName = model.FullName, UserName = model.Email, Email = model.Email, UserTypeId = model.SelectedUserTypeId };
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    await SignInManager.SignInAsync(user, isPersistent:false, rememberBrowser:false);

                    unitOfWork.SystemAntenna.OnUserRegisterationSuccess(user.Id);
